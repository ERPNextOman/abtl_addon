...
def validate(doc,method):
    # check_reserved_qty(doc)
    # items_zero_after_validate(doc)
    create_DN(doc)


#Check Reserved Qty
def check_reserved_qty(doc):
    for i in doc.items:
        reserved_qty = frappe.db.sql(f"""select reserved_qty from `tabBin` where item_code = '{i.item_code}' AND warehouse = '{i.warehouse}'""",as_dict=1)
        reserved_qty = str(reserved_qty)
        reserved_qty = reserved_qty.replace("[{'reserved_qty':","")
        reserved_qty = reserved_qty.replace("}]","")
        
        sum_qty = float(reserved_qty) + float(i.qty)
        
        if float(i.actual_qty) <= float(reserved_qty):
            frappe.throw("<b style='color:red;'>"+ i.item_code + "</b>"+" Reserved Qty is more than Available Qty")

        elif float(i.actual_qty) <= sum_qty:
            frappe.throw("<b style='color:red;'>"+ i.item_code + "</b>"+" Reserved Qty is more than Available Qty")
